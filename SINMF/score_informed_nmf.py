...
def score_informed_nmf(w_init, h_init, signal, pitches):
    ...
    W, H = decompose_custom(signal.X, n_components=n_components, sort=False, transformer=transformer, W=w_init,
                            H=h_init)
...
#TODO
def khr_get_fake_midi(filename):
    files = []  # 用于存储模拟的 "MIDI 文件"
    pitches = []  # 用于存储所有的音符编号（去重前）

    with open(filename, 'r') as csvfile:
        reader = csv.reader(csvfile)
        next(reader)  # 跳过表头（如果存在表头）
        for row in reader:
            onset, offset, pitch = row  # 读取每一行的 onset, offset, pitch
            pitch = int(pitch)  # 转换 pitch 为整数
            pitches.append(pitch)  # 将 pitch 添加到列表
            files.append({'onset': float(onset), 'offset': float(offset), 'pitch': pitch})  # 模拟MIDI结构

    # 返回所有音符数据和去重后的音符列表（按顺序排序）
...
def initialize_activations(signal, mid, pitches, tol_on, tol_off, by_channel=False, max_time=None):
    ...
    for msg in mid:
        ...
        if msg.type == 'note_on' and msg.velocity != 0:
            ...
            start_frame, end_frame = librosa.time_to_frames([note_on_since - tol_on, current_time + tol_off],
...
def initialize_activations_with_onset_templates(signal, mid, pitches, tol_on, tol_off, max_time=None):
    ...
    for msg in mid:
        ...
        if msg.type == 'note_on':
            on_since[msg.note] = current_time
            component = pitch_to_component(msg.note, pitches)
            start_frame, end_frame = librosa.time_to_frames([current_time - tol_on, current_time + tol_on],
                                                            sr=signal.sr, hop_length=signal.fft_hop_length)
            start_frame = max(start_frame, 0)
            end_frame = min(end_frame, n_samples - 1)
...
def score_informed_nmf_with_onset_templates(w_init, h_init, signal, pitches):
    ...
    W, H = decompose_custom(signal.X, n_components=n_components * 2, sort=False, transformer=transformer, W=w_init,
                            H=h_init)
...
def initialize_activations_from_score(signal, pitches, score, tol_on, tol_off):
    ...
    for note in score:
        ...
        start_frame, end_frame = librosa.time_to_frames([note[1] - tol_on, note[2] + tol_off], sr=signal.sr,
                                                        hop_length=signal.fft_hop_length)
        start_frame = max(start_frame, 0)
        end_frame = min(end_frame, n_samples - 1)
        H_init[component, start_frame:end_frame] = 1
...
def initialize_activations_params(signal: Signal, mid, init_params: ScoreInitialization, by_channel=False) -> Tuple[
